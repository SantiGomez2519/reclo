        ...
        <div class="col-md-12">
            <div class="card admin-card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <a href="{{ route('admin.customusers.edit', $viewData['customUser']->getId()) }}" class="btn btn-warning me-2">
                ...
                <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <p class="mb-1">
                                                    <span class="badge bg-info">{{ __('admin.payment_' . str_replace(' ', '_', strtolower($viewData['customUser']->getPaymentMethod()))) }}</span>
                                                </p>
                                            </div>

                                            <div class="mb-3">
                                                <strong class="text-muted">{{ __('admin.member_since') }}:</strong>
                                                <p class="mb-1">{{ date('M d, Y', strtotime($viewData['customUser']->getCreatedAt())) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="card text-center bg-primary text-white">
                                        <div class="card-body">
                                            <h3 class="card-title">{{ $viewData['customUser']->getProducts()->count() }}</h3>
                                ...
                                <div class="col-md-4">
                                    <div class="card text-center bg-success text-white">
                                        <div class="card-body">
                                            <h3 class="card-title">{{ $viewData['customUser']->getOrders()->count() }}</h3>
                                ...
                                <div class="col-md-4">
                                    <div class="card text-center bg-info text-white">
                                        <div class="card-body">
                                            <h3 class="card-title">{{ $viewData['customUser']->getReviews()->count() }}</h3>
                    ...
                    @if($viewData['customUser']->getProducts()->count() > 0)
                        <div class="mb-4">
                            <h5 class="mb-3">
                                <i class="fas fa-shopping-bag me-2"></i>{{ __('admin.products_listed') }} ({{ $viewData['customUser']->getProducts()->count() }})
                            </h5>
                                        ...
                                        @foreach($viewData['customUser']->getProducts()->take(5) as $product)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @else
                                                        <div class="customer-product-placeholder d-flex align-items-center justify-content-center">
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.products.show', $product->getId()) }}" 
                                                       class="btn btn-sm btn-outline-primary" title="{{ __('admin.view_product') }}">
                            ...
                            @if($viewData['customUser']->getProducts()->count() > 5)
                                <div class="text-center">
                                    <small class="text-muted">{{ __('admin.showing_products', ['shown' => 5, 'total' => $viewData['customUser']->getProducts()->count()]) }}</small>
                                </div>
                            @endif
                        </div>
                    @endif
                    ...
                    @if($viewData['customUser']->getOrders()->count() > 0)
                        <div class="mb-4">
                            <h5 class="mb-3">
                                <i class="fas fa-shopping-cart me-2"></i>{{ __('admin.recent_orders') }} ({{ $viewData['customUser']->getOrders()->count() }})
                            </h5>
                            <div class="table-responsive">
                                <table class="table table-sm table-striped">
                                    <thead class="table-dark">
                                    ...
                                    <tbody>
                                        @foreach($viewData['customUser']->getOrders()->take(5) as $order)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge bg-info">{{ __('admin.status_' . $order->getStatus()) }}</span>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            @if($viewData['customUser']->getOrders()->count() > 5)
                                <div class="text-center">
                                    <small class="text-muted">{{ __('admin.showing_orders', ['shown' => 5, 'total' => $viewData['customUser']->getOrders()->count()]) }}</small>
                                </div>
                            @endif
                        </div>
                    @endif
                    ...
                    @if($viewData['customUser']->getReviews()->count() > 0)
                        <div class="mb-4">
                            <h5 class="mb-3">
                                <i class="fas fa-star me-2"></i>{{ __('admin.recent_reviews') }} ({{ $viewData['customUser']->getReviews()->count() }})
                            </h5>
                            <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="card">
                                            <div class="card-body">
                                                ...
                                                <p class="card-text small">{{ Str::limit($review->getComment(), 100) }}</p>
                                                <small class="text-muted">{{ date('M d, Y', strtotime($review->getCreatedAt())) }}</small>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            @if($viewData['customUser']->getReviews()->count() > 3)
                                <div class="text-center mt-3">
                                    <small class="text-muted">{{ __('admin.showing_reviews', ['shown' => 3, 'total' => $viewData['customUser']->getReviews()->count()]) }}</small>
                                </div>
                            @endif
                        </div>
                    @endif

                    @if($viewData['customUser']->getProducts()->count() == 0 && $viewData['customUser']->getOrders()->count() == 0 && $viewData['customUser']->getReviews()->count() == 0)
