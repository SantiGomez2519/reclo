...
<head>
    ...
    <title>{{ config('app.name', 'Reclo') }} - {{ __('adminLayout.admin') }}</title>
        ...
        <nav class="navbar navbar-expand-md navbar-dark admin-header shadow-sm">
            <div class="container">
                <a class="navbar-brand fw-bold" href="{{ route('admin.dashboard') }}">
                    <i class="fas fa-shield-alt me-2"></i>{{ config('app.name', 'Reclo') }} {{ __('adminLayout.admin') }}
                </a>
                        ...
                        <form id="admin-logout" action="{{ route('admin.logout') }}" method="POST">
                            <a role="button" class="nav-link text-white" 
                              onclick="document.getElementById('admin-logout').submit();">
                                <i class="fas fa-sign-out-alt me-1"></i>{{ __('adminLayout.logout') }}
                            </a>
                            @csrf 
                        </form>
                    @endauth
