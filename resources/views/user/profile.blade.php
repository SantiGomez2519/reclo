                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-8">
                                        <span
                                            class="badge bg-secondary">{{ __('user.' . str_replace(' ', '_', strtolower($viewData['user']->getPaymentMethod()))) }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="row mt-4">
                                    <div class="col-md-6 text-center">
                                        <div class="border rounded p-2 bg-light">
                                            ...
                                            <small class="text-muted">{{ __('user.sales') }}</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <ul class="nav nav-tabs" id="profileTabs" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link" id="sales-tab" data-bs-toggle="tab" href="#sales" role="tab">
                                    <i class="fas fa-store me-1"></i>{{ __('user.my_sales') }}
                            ...
                            <div class="tab-pane fade" id="sales" role="tabpanel">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-7">
                                                    ...
                                                    <div class="row mb-2">
                                                        ...
                                                        <div class="col-md-6">
                                                            <strong>{{ __('user.sold_on') }}:</strong>
                                                            {{ $product->getUpdatedAt()->translatedFormat('M j, Y') }}
                                                        </div>
                                                    </div>
                                                    <div class="row mb-2">
                                                        <div class="col-md-6">
                                                            <strong>{{ __('user.buyer') }}:</strong>
                                                            {{ $product->order->buyer->getName() ?? __('user.unknown') }}
                                                        </div>
                                                        <div class="col-md-6">
                                                            <strong>{{ __('user.category') }}:</strong>
                                                            <span
                                                                class="badge bg-primary">{{ $product->getCategory() }}</span>
                                                        </div>
                                                    </div>
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <strong>{{ __('user.condition') }}:</strong>
                                                            <span
                                                                class="badge bg-info">{{ $product->getCondition() }}</span>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <strong>{{ __('user.size') }}:</strong>
                                                            <span
                                                                class="badge bg-secondary">{{ $product->getSize() }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-2 text-center">
                                                    @if ($product->review)
                                                        <div class="rating">
                                                            <h6>{{ __('user.rating') }}:</h6>
                                                            @for ($i = 1; $i <= 5; $i++)
                                                                <i
                                                                    class="fas fa-star {{ $i <= $product->review->rating ? 'text-warning' : 'text-muted' }}"></i>
                                                            @endfor
                                                            <p class="text-muted small mt-2">
                                                                "{{ Str::limit($product->review->comment, 50) }}"</p>
                                                        </div>
                                                    @else
                                                        <div class="text-muted">
                                                            <i class="fas fa-comment-slash fa-2x mb-2"></i>
                                                            <p class="small">{{ __('user.no_rating_yet') }}</p>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                ...
                                @empty
                                    <div class="text-center py-4 mt-3">
                                        ...
                                        <h5 class="text-muted">{{ __('user.no_sales_yet') }}</h5>
                                        <p class="text-muted">{{ __('user.your_sales_history_empty') }}</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
